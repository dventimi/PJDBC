...
public abstract class AbstractProxyConnection implements Connection {
    ...
    public AbstractProxyConnection (Connection delegate, Driver driver, String url, Properties info, List<Connection> delegates) 
	throws SQLException {
	this(delegate, driver, url, info);
	for (Connection c : delegates) addDelegate(c);}

    public AbstractProxyConnection (Connection delegate, Driver driver, String url, Properties info, Connection... delegates) 
	throws SQLException {
	this(delegate, driver, url, info);
	for (Connection c : delegates) addDelegate(c);}

    public boolean addDelegate (Connection conn) 
	throws SQLException {
	return this.delegates.add(conn);}

    public boolean removeDelegate (Connection conn) 
	throws SQLException {
